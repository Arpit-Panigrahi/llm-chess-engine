// board.c
// Core board helpers: FEN parsing, board reset, material and
// piece-list rebuild, and deep consistency checking of S_BOARD.
...
void BoardToFen(const S_BOARD *pos, char *fen) {
	int rank, file, sq, piece, empty;
	char *p = fen;
	...
	*p++ = ' ';
	...
	if(pos->castlePerm) {
		if(pos->castlePerm & WKCA) *p++ = 'K';
		if(pos->castlePerm & WQCA) *p++ = 'Q';
		if(pos->castlePerm & BKCA) *p++ = 'k';
		if(pos->castlePerm & BQCA) *p++ = 'q';
	} else {
		*p++ = '-';
	}
	...
	*p++ = ' ';
	...
	if(pos->enPas != NO_SQ) {
		*p++ = 'a' + FilesBrd[pos->enPas];
		*p++ = '1' + RanksBrd[pos->enPas];
	} else {
		*p++ = '-';
	}

	sprintf(p, " %d %d", pos->fiftyMove, (pos->hisPly / 2) + 1);
}
