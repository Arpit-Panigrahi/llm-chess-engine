...
def run_fast_clamped_benchmark():
    print("=" * 75, flush=True)
    print("  LLM Chess Engine — Fast Clamped Decoding (num_predict=6) Live Benchmark", flush=True)
    print("=" * 75, flush=True)

    out_dir = "reports/fast_clamped_benchmark"
    ...
    test_conditions = [
        {
            "tag": "t08_fast_clamped_dmc",
            "name": "Single-Stage (Fast Clamped Quoted DMC)",
            "num_games": 10,
            "max_turns": 6,
            "temp": 0.8,
            "mode": "single-stage",
    ...
    try:
        for cond_idx, cond in enumerate(test_conditions, 1):
            ...
            print(f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", flush=True)
            print(f"  [{cond_idx}/3] EVALUATING: {name}", flush=True)
            ...
            for g in range(1, num_games + 1):
                ...
                while not board.is_game_over() and turn_num < max_turns and not game_aborted:
                    turn_num += 1
                ...
                status_str = "Aborted" if game_aborted else "Completed"
                print(f"  Game {g:2d}/{num_games}: {status_str} | Legal {g_legal}/{g_total} ({rate:.0f}%) | Time: {g_duration:.1f}s", flush=True)
    ...
    print("=" * 75, flush=True)
    print("  ✅ Benchmark Complete! Results saved to: reports/fast_clamped_benchmark/", flush=True)
    print("=" * 75, flush=True)

if __name__ == "__main__":
    run_fast_clamped_benchmark()
